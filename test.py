...
# env -> actor
# actor -> trajactory // local_len, local_bs per actor
# ㄴ trajactory : state, action, reward, lstm_state
# trajactory -> batch
# learner <- trajatory, V-trace and optimizer (RMS)
...
def actor(idx, ps, data, env, args):
    ...
    while True:
        ...
        with torch.no_grad():
            while True:
                if rollout.length == length + 1: # length 가 n step
