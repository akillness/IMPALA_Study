...
def learner(model, data, ps, args, terminate_event):
    ...
    while not terminate_event.is_set():
        ...
        unroll_len = trajectory.length
        discounts = np.array([pow(args.gamma, i) for i in range(unroll_len-1)])
        # discounts = np.repeat(discounts, unroll_len).reshape(unroll_len, -1)
        discounts = torch.tensor(discounts,dtype=torch.float32).to(device)
        '''        
        discounts = (~dones).float() * args.gamma
        '''
        # 선택된 행동의 로그 확률 추출
        ...
        # policy_loss = (pg_cross_entropy * pg_advantages.detach()).mean()
        ...
        # entropy_loss = entropy_cost * -(-F.softmax(target_logits, 1) * F.log_softmax(target_logits, 1)).sum(-1).mean()
